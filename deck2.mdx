import { Image, Notes, Appear, Embed } from '@mdx-deck/components'  
import { FullScreenCode, Invert } from 'mdx-deck/layouts'

import future from '@mdx-deck/themes/future'
import aspect from '@mdx-deck/themes/aspect'
import highlight from '@mdx-deck/themes/syntax-highlighter-prism'
import customTheme from './theme'

export const themes = [future, aspect, highlight, customTheme]

## The Pragmatic Front-end Tester

---

I wanted to talk about testing the front-end.

<Notes>
  "Who's writing FE apps? Who's testing? Are you confiden? Would you ship
  without manually testing?"

Over the years, FE apps have increased in complexity, responsibility has moved 
from the backend to the FE. 
</Notes>

---

...and I wanted to do it following a pragmatic approach.

<Notes>
  Diapos en ingles porque estaba preparando la charla en ingles, no por postureo.
  Bueno y por postureo un poco también.
</Notes>

---

## Some Facts

<Appear>

I have 15 minutes 🏃‍♂️

<s>Why should we test?</s>

<s>Testing beyond avoiding regressions</s>

<s>Visual regression testing</s>

Focused on front-end apps

</Appear>

<Notes>
  Focus on avoiding regressions, not discovering best ways of developing a feature

  Frontend apps: Ni Node ni librerias ni otros contextos
</Notes>

---

The testing pyramid?


```html
          ^
         / \
        /   \
       / e2e \
      /_______\
     /         \
    /    int    \
   /_____________\
  /               \
 /       unit      \
/___________________\
```


---

<Image src="images/drake-nah.png" size="contain" />

---

Testing your components in a real(ish) environment...

interacting with them as a user would...

...and focusing on the amount of confidence they give?

---

<Image src="images/drake-yea.jpg" size="contain" />

---

# 👋

## Adrià Fontcuberta

Senior(?) Front-end Engineer @ Holaluz

`afontcu.dev` &nbsp;&nbsp;·&nbsp;&nbsp; `@afontcu_` &nbsp;&nbsp;·&nbsp;&nbsp; `npx afontcu`

<Notes>
  Holaluz: Electrica BCN tenemos 200.000 clientes donde vendemos energia verde y solo energia verde, y pretendemos crear una relación de confianza con nuestros clientes, algo un poco inusual en el sector como ya sabréis.
</Notes>

---

So...

I'm sorry I'm gonna use a Counter component

# 🤷‍♀️

---

<FullScreenCode>

```jsx
// Hi this is Vue

const Counter = {
  render() {
    return (
      <div>
        <p class="count">You clicked {this.count} times</p>
        <button onClick={this.increment}>increment</button>
      </div>
    )
  },

  data: () => ({ count: 0 }),

  methods: {
    increment() { this.count++ },
  },
}
```

</FullScreenCode>

---

<FullScreenCode>

```jsx
// Hi this is React

const Counter = () => {
  const [count, setCount] = useState(0)

  const increment = () => setCount(count + 1)

  return (
    <div>
      <p className="count">You clicked {count} times</p>
      <button onClick={increment}>increment</button>
    </div>
  )
}
```

</FullScreenCode>

---


```
import { mount } from '@vue/test-utils'
import Counter from './Counter'

test('increments value on click', () => {
  const wrapper = mount(Counter)
  const paragraph = wrapper.find('.count')

  expect(paragraph.text()).toBe('You clicked 0 times')

  wrapper.vm.increment()
  wrapper.vm.increment()

  expect(paragraph.text()).toBe('You clicked 2 times')
})
```
 
---

<FullScreenCode>

```jsx
const Counter = {
  render() {
    return (
      <div>
        <p className="count">You clicked {this.count} times</p>
        <button onClick={this.increment}>increment</button>
      </div>
    )
  },

  data: () => ({ count: 0 }),

  methods: {
    increment() { this.count++ },
  },
}
```

</FullScreenCode>

---

<FullScreenCode>

```jsx
const Counter = {
  render() {
    return (
      <div>
        <p className="count">You clicked {this.count} times</p>
        <button onClick={() => this.count++}>increment</button>
      </div>
    )
  },

  data: () => ({ count: 0 }),
}
```

</FullScreenCode>

---

<FullScreenCode>

```jsx
const Counter = {
  render() {
    return (
      <div>
        <p className="counter">You clicked {this.count} times</p>
        <button onClick={() => this.count++}>increment</button>
      </div>
    )
  },

  data: () => ({ count: 0 }),
}
```

</FullScreenCode>

---

<span style={{ fontSize: '11rem' }}>❌</span>

```py
find() did not return .counter, cannot call text() on empty Wrapper
```

```py
TypeError: wrapper.vm.increment is not a function
```

---

<Image src="images/kalisi.jpg" size="cover" />

---

False negative.

---

This undermines our confidence in testing.

_"Tests fail at random times"_

---

```js
undefined = () => {}
```

---

## ⚠️ Implementation details ⚠️

---

<Appear>

Your test does something its **consumers** won't.

Your test breaks on **refactor**.

</Appear>


<Notes>
  If test is doing something different from user & developer, it becomes a third
  consumer.
  User: input interactions, output side effects
  Dev: input props, output elements, event

  Refactor: changing internal implementation without modifying external behavior.

  Exemple canviar nom de mètode.

  This undermines our confidence in testing. They fail at random times.
</Notes>

---

`dom-testing-library`

<Appear>

> “The more your tests resemble the way your software is used, the more confidence they can give you.” - _Kent C. Dodds_

</Appear>

---

`vue-testing-library`

`react-testing-library`

`svelte-testing-library`

`angular-testing-library`

...

<Notes>

Built on top of vue test utils. Hides methods that get you closer to testing
implementation details, while providing user-focused helpers.

If you want to avoid impl details, use tools that help you out.

Helps you querying for nodes similarly to how users would find them.

</Notes>

---

```js
import { render, fireEvent } from 'vue-testing-library'
import Counter from './Counter'

test('increments value on click', async () => {
  const { getByText } = render(Counter)

  getByText(/you clicked 0 times/i)                              

  const button = getByText(/increment/i)

  await fireEvent.click(button) 
  await fireEvent.click(button) 

  getByText(/you clicked 2 times/i)
})
```

---

```js
import { render, fireEvent } from 'vue-testing-library'
import Counter from './Counter'

test('increments value on click', async () => {
  const { getByText } = render(Counter)

  getByText(/you clicked 0 times/i)      // query as a user would

  const button = getByText(/increment/i) // query as a user would

  await fireEvent.click(button)          // act as a user would
  await fireEvent.click(button)          // act as a user would

  getByText(/you clicked 2 times/i)      // query as a user would
})
```

---


```jsx
import { render, fireEvent } from 'react-testing-library'
import Counter from './Counter'

test('increments value on click', async () => {
  const { getByText } = render(<Counter />)

  getByText(/you clicked 0 times/i)      // query as a user would

  const button = getByText(/increment/i) // query as a user would

  await fireEvent.click(button)          // act as a user would
  await fireEvent.click(button)          // act as a user would

  getByText(/you clicked 2 times/i)      // query as a user would
})
```

---

<span style={{ fontSize: '11rem' }}>✅</span>

---

(kinda) framework agnostic

# 👍

---

Let me show you something else.

---

Something that turned writing tests... into a lovely activity.

---

# Cypress ❤️

<Notes>
  Is an amazing tool to write end-to-end tests. It can test anything that runs
  on a browser (including Vue).

  It includes a ton of tools and features out-of-the-box: mocking/stubbing,
  assertions, DOM selectors...

  Focus on DX. Meant to be used during the development cycle.
</Notes>

---

So...

I'm sorry I'm gonna use a Todo app

# 🤷‍♀️

---

XHR stubbing

Custom commands

`cypress-testing-library`

---

```js
it('adds 3 todos', () => {
  cy.visit('/')

  cy.getByLabelText(/what needs to be done?/i)
    .type('follow @afontcu_ on Twitter{enter}')
    .type('learn testing with Cypress{enter}')
    .type('stop making demos with Todo apps{enter}')
  
  cy.getAllByTestId('todolist-item')
    .should('have.length', 3)
})
```

---


```jsx
<label data-testid="todolist-item">
  {this.props.todo.title}
</label>
```

---


```js
it('adds 3 todos', () => {
  cy.visit('/')

  cy.getByLabelText(/what needs to be done?/i)
    .type('follow @afontcu_ on Twitter{enter}')
    .type('learn testing with Cypress{enter}')
    .type('stop making demos with Todo apps{enter}')
  
  cy.getAllByTestId('todolist-item')
    .should('have.length', 3)
})
```

---

Hey, Adri.

Why would I write "component" tests if I can use Cypress?

---

That's a pretty good question.

---

<p style={{textAlign:'left'}}>Component tests tend to be **cheaper**.</p>

<p style={{textAlign:'left'}}>Component tests tend to be **faster**.</p>

<p style={{textAlign:'left'}}>Component tests tend to be **more focused**.</p>

<Notes>
  Cheaper to write and run.

  Better balance between cost, confidence and scope.

However, we are kinda getting there with tools such as Cypress.
</Notes>

---

## Unit tests?

<Appear>

So you have Entities, Use cases or similar? Test the hell out of it.

They help me uncover poorly designed code.

I'd use them when working with plain JS files and pure functions.

</Appear>

<Notes>
  Avoiding regressions is a nice side effect, but not the goal itself.
</Notes>

---


<FullScreenCode>

```js
export async function handleAddRegistration({
  user,
  registration,
  isExistingUser,
  validateUser,
  createNewUser,
  createNewRegistration,
  respondWithError,
  respondWithSuccess,
}) {
  if (await isExistingUser(user)) {
    await validateUser(user)
  } else {
    await createNewUser(user)
  }

  try {
    await createNewRegistration(registration)
  } catch (err) {
    await respondWithError(err)
  }

  await respondWithSuccess()
}
```

</FullScreenCode>

---

<FullScreenCode>

```js
test('Create new registration for an existing user', async () => {
  const user = {}
  const registration = {}
  const isExistingUser = jest.fn(() => true)
  const validateUser = jest.fn()
  const createNewUser = jest.fn()
  const createNewRegistration = jest.fn()
  const respondWithError = jest.fn()
  const respondWithSuccess = jest.fn()

  await handleAddRegistration({
    user,
    registration,
    isExistingUser,
    validateUser,
    createNewUser,
    createNewRegistration,
    respondWithError,
    respondWithSuccess,
  })

  expect(isExistingUser).toHaveBeenCalledTimes(1)
  expect(isExistingUser).toHaveBeenCalledWith(user)

  expect(validateUser).toHaveBeenCalledTimes(1)
  expect(validateUser).toHaveBeenCalledWith(user)

  expect(createNewUser).not.toHaveBeenCalled()

  expect(isExistingUser).toHaveBeenCalledTimes(1)

  expect(createNewRegistration).toHaveBeenCalledWith(registration)

  expect(respondWithError).not.toHaveBeenCalled()
  expect(respondWithSuccess).toHaveBeenCalledTimes(1)
})
```

</FullScreenCode>

---

## Recap

---

A pragmatic front-end testing toolbelt

`[insert-your-framework]-testing-library`

Cypress

---

General tips for the pragmatic (front-end) tester

---

# tip #1 

Use **Eslint**. Seriously, it is a testing tool.

Then, turn off the stylistic rules and install Prettier.

---

# tip #2

You found a bug? Great. 👍

<Appear>

Now, **write a test before** fixing it and watch it fail.

Then fix it. Make sure the bug stays dead.

</Appear>

---

# tip #3

Having a hard time testing a piece of code? Do you need to mock half the app? That's a bad smell.

<Appear>

If it's hard to test, it's probably **doing too much**.

If it's doing to much, it's probably hard to **understand**.

If it's hard to understand, it's hard to **maintain**.

</Appear>

<Notes>
  Testing higlights pieces of code that could use some refactor.

If a test inflicts pain, we tend to blame the test - even when the root cause is in the code's design.
</Notes>

---

## Wrapping up

<Appear>

Write tests, but **not too many**. Focus on ROI.

I'd use Cypress to cover common happy paths.

I'd add several **component tests** to critical parts and edge cases.

I'd also write **unit** tests to design small modules and functions.

</Appear>

---

## To learn more

[frontend-testing.now.sh](https://frontend-testing.now.sh)

[github.com/afontcu/awesome-learning#testing](https://github.com/afontcu/awesome-learning#testing)

[docs.cypress.io](https://docs.cypress.io)

[testing-library.com](http://testing-library.com)

---

# Thank you! 👋

`afontcu.dev` &nbsp;&nbsp;·&nbsp;&nbsp; `@afontcu_` &nbsp;&nbsp;·&nbsp;&nbsp; `npx afontcu`

---

### oh btw we r hiring k thx bye 👋

👉 http://tiny.cc/holaluz-frontend 👈

_(more openings at: [holaluz.com/jobs](https://holaluz.com/jobs))_
