...
<Notes>
Tengo la necesidad de daros las gracias. Sois unos y unas valientes.
...
afontcu.dev &nbsp;&nbsp;·&nbsp;&nbsp; <strike>@afontcu_</strike>
</p>
...
function renderCounter() {
  ...
  const click = () =>
    fireEvent.click(utils.getByText(/increment/i))

  const getCounterText = num =>
    utils.getByText(new RegExp('clicked ' + num + ' times'))
...
test('increments value on click', async () => {
  const { click, getCounterText } = renderCounter(Counter)    

  expect(getCounterText(0)).toBeTruthy()

  await click()
  await click()

  expect(getCounterText(2)).toBeTruthy()
})
```
...
function renderCounter() {
  ...
  return { click, getCounterText, ...utils }
}
```
...
function renderCounter() {
  const utils = render(Counter, { store })
...
<Image src="images/abadia.jpeg" size="cover" style={{
  marginLeft: '-10vw'
}}/>

<Notes>

Javier Abadía – Refactoring
